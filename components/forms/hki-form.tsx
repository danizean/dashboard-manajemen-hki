...
const fileSchema = z.instanceof(FileList)
  .optional()
  .refine(files => !files || files.length === 0 || files[0].size <= MAX_FILE_SIZE_BYTES, `Ukuran file maksimal ${MAX_FILE_SIZE_MB}MB.`)
  .refine(files => !files || files.length === 0 || ACCEPTED_FILE_TYPES.includes(files[0].type), 'Hanya file format .pdf.');
...
const hkiSchema = z.object({
  ...
  tahun_fasilitasi: z.coerce.number({ invalid_type_error: 'Tahun wajib dipilih.' }).int().min(new Date().getFullYear() - 25, 'Tahun tidak valid.').max(new Date().getFullYear() + 1, `Tahun tidak boleh melebihi ${new Date().getFullYear() + 1}.`),
  keterangan: z.string().optional().nullable(),
  id_jenis_hki: z.string({ required_error: 'Jenis HKI wajib dipilih.' }).min(1, 'Jenis HKI wajib dipilih.'),
...
const FileUploader = memo(({ control, formId, isSubmitting, initialPdf, onRemoveExisting }: any) => {
  const [isDragging, setIsDragging] = useState(false);
  const selectedFile = useWatch({ control, name: 'sertifikat_pdf' });
  const showExistingFile = !!initialPdf && !selectedFile?.[0];

  return (
    <FormField control={control} name="sertifikat_pdf" render={({ field: { onChange, onBlur, name, ref } }) => (
      <FormItem>
        <FormLabel>Sertifikat PDF (Opsional)</FormLabel>
        <FormControl>
          <Input type="file" accept=".pdf" className="hidden" id={`${formId}-file-upload`} name={name} ref={ref} onBlur={onBlur} onChange={(e) => onChange(e.target.files)} disabled={isSubmitting} />
        </FormControl>
        <label htmlFor={`${formId}-file-upload`} className={cn('flex items-center justify-center w-full p-4 border-2 border-dashed rounded-lg cursor-pointer transition-colors', isDragging && 'border-primary bg-primary/10', isSubmitting ? 'bg-muted/50 cursor-not-allowed' : 'hover:bg-muted/50')} onDragEnter={() => setIsDragging(true)} onDragLeave={() => setIsDragging(false)} onDrop={() => setIsDragging(false)}>
          <div className="text-center">
            <FileText className="mx-auto h-10 w-10 text-muted-foreground" />
            <p className="mt-2 text-sm text-muted-foreground"><span className="font-semibold text-primary">Klik untuk memilih file</span> atau seret file ke sini</p>
        ...
        <FormDescription>{initialPdf && 'Mengunggah file baru akan menggantikan file yang lama.'}</FormDescription>
        <FormMessage />
      </FormItem>
    )} />
  );
...
export const HKIForm = memo(({ id, initialData, mode, ...props }: HKIFormProps) => {
  ...
  const showKelasField = useMemo(() => {
    const selectedJenis = jenisOptions.find(opt => String(opt.id_jenis_hki) === selectedJenisId);
  ...
  const onSubmit = useCallback(async (data: HKIFormData) => {
    ...
    try {
      ...
      toast.success(`Data HKI berhasil di${actionText}!`, { id: toastId, description: `Entri untuk "${data.nama_hki}" telah disimpan.` });
      onSuccess?.(result.data);
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : `Terjadi kesalahan saat ${actionText} data.`;
      toast.error(errorMessage, { id: toastId, description: 'Silakan periksa kembali isian Anda atau coba beberapa saat lagi.' });
  ...
  return (
    <Form {...form}>
      <form id={id} onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-6">
        <fieldset disabled={isSubmitting} className="space-y-6">
          <Card>
            <CardHeader><CardTitle>Informasi Utama HKI</CardTitle></CardHeader>
            <CardContent className="space-y-6">
              <FormField name="nama_hki" control={control} render={({ field }) => (<FormItem><FormLabel>Nama HKI *</FormLabel><FormControl><Input placeholder="Contoh: Merek Kopi 'Sleman Jaya'" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField name="jenis_produk" control={control} render={({ field }) => (<FormItem><FormLabel>Jenis Produk</FormLabel><FormControl><Input placeholder="Contoh: Makanan Olahan, Minuman, Jasa Konsultasi" {...field} value={field.value ?? ''} /></FormControl><FormDescription>Jelaskan produk atau jasa yang terkait.</FormDescription><FormMessage /></FormItem>)} />
            </CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>Detail Pemohon</CardTitle></CardHeader>
            <CardContent className="space-y-6">
              <FormField name="nama_pemohon" control={control} render={({ field }) => (<FormItem><FormLabel>Nama Pemohon *</FormLabel><FormControl><Input placeholder="Nama lengkap perorangan atau perusahaan" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField name="alamat" control={control} render={({ field }) => (<FormItem><FormLabel>Alamat Pemohon</FormLabel><FormControl><Textarea rows={3} placeholder="Alamat lengkap sesuai KTP/domisili..." {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
            </CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>Data Administrasi</CardTitle></CardHeader>
            <CardContent className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <FormField name="id_jenis_hki" control={control} render={({ field }) => (<FormItem><FormLabel>Jenis HKI *</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih jenis HKI" /></SelectTrigger></FormControl><SelectContent>{jenisOptions.map((opt) => (<SelectItem key={opt.id_jenis_hki} value={String(opt.id_jenis_hki)}>{opt.nama_jenis_hki}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                <FormField name="id_status" control={control} render={({ field }) => (<FormItem><FormLabel>Status HKI *</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih status HKI" /></SelectTrigger></FormControl><SelectContent>{statusOptions.map((opt) => (<SelectItem key={opt.id_status} value={String(opt.id_status)}>{opt.nama_status}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                <FormField name="tahun_fasilitasi" control={control} render={({ field }) => (<FormItem><FormLabel>Tahun Fasilitasi *</FormLabel><Select onValueChange={(value) => field.onChange(Number(value))} defaultValue={String(field.value)}><FormControl><SelectTrigger><SelectValue placeholder="Pilih tahun" /></SelectTrigger></FormControl><SelectContent><SelectGroup>{yearOptions.map((year) => (<SelectItem key={year.value} value={year.value}>{year.label}</SelectItem>))}</SelectGroup></SelectContent></Select><FormMessage /></FormItem>)} />
              </div>
              <FormField name="id_pengusul" control={control} render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel>Pengusul (OPD) *</FormLabel>
                  <Suspense fallback={ComboboxFallback}>
                    <LazyCombobox options={pengusulOptions} value={field.value} onChange={field.onChange} placeholder="Pilih OPD pengusul..." searchPlaceholder="Cari OPD..." />
                  </Suspense>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              {showKelasField && (
                <FormField name="id_kelas" control={control} render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>Kelas HKI (untuk Merek)</FormLabel>
                     <Suspense fallback={ComboboxFallback}>
                        <LazyCombobox options={kelasOptions} value={field.value ?? ''} onChange={field.onChange} placeholder="Pilih Kelas HKI (1-45)..." searchPlaceholder="Cari kelas (cth: 1, 35, atau Iklan)..."/>
                     </Suspense>
                    <FormMessage />
                  </FormItem>
                )} />
              ...
              <FormField name="keterangan" control={control} render={({ field }) => (<FormItem><FormLabel>Keterangan</FormLabel><FormControl><Textarea rows={3} placeholder="Informasi tambahan, catatan internal, atau detail lain..." {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
              <FileUploader control={control} formId={id} isSubmitting={isSubmitting} initialPdf={!isDeletingFile ? initialData?.sertifikat_pdf : null} onRemoveExisting={handleRemoveExistingFile}/>
            </CardContent>
          </Card>
        </fieldset>
      </form>
