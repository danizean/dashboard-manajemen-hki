...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from(HKI_TABLE).select(
    ...
    if (params.search) {
      ...
      query = query.or(
        `nama_hki.ilike.%${params.search}%,pemohon.nama_pemohon.ilike.%${params.search}%,jenis_produk.ilike.%${params.search}%`
      )
    }
    if (params.jenisId) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (insertError || !newHki) {
      throw new Error(
        `Gagal menyimpan data HKI: ${insertError?.message || 'Data tidak kembali.'}`
      )
    }
