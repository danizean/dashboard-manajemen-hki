...
async function authorizeAdmin(supabase: SupabaseClient<Database>) { // <-- Tipe ini sekarang dikenali dengan benar
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from(HKI_TABLE).select(`*, pemohon(*), jenis:jenis_hki(*), status_hki(*), pengusul(*), kelas:kelas_hki(*)`, { count: 'exact' });
    ...
    query = query.order(params.sortBy, { ascending: params.sortOrder === 'asc' }).range(from, from + params.pageSize - 1);

    const { data, error, count } = await query;
    if (error) throw error;
    
    const response = NextResponse.json({ data: data || [], totalCount: count ?? 0 });
    return response;
...
async function getPemohonId(supabase: ReturnType<typeof createClient>, nama: string, alamat: string | null): Promise<number> {
...
export async function POST(request: NextRequest) {
    ...
    try {
      ...
      if (insertError) throw new Error(`Gagal menyimpan data HKI: ${insertError.message}`);
