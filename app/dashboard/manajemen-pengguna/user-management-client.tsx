...
import { Loader2, MoreHorizontal, Pen, Plus, Shield, Trash2, Users } from 'lucide-react'
...
export function UserManagementClient({
  ...
  const [modalState, setModalState] = useState<{ isOpen: boolean; user?: UserProfile }>({ isOpen: false });
  const [deleteAlert, setDeleteAlert] = useState<{ isOpen: boolean; user?: UserProfile }>({ isOpen: false });
  const router = useRouter();

  const openModal = (user?: UserProfile) => setModalState({ isOpen: true, user });
  const closeModal = () => setModalState({ isOpen: false });

  const openDeleteAlert = (user: UserProfile) => setDeleteAlert({ isOpen: true, user });
  const closeDeleteAlert = () => setDeleteAlert({ isOpen: false });

  const handleDelete = async () => {
    if (!deleteAlert.user) return;
    const toastId = toast.loading(`Menghapus pengguna ${deleteAlert.user.email}...`);

    try {
      const res = await fetch(`/api/users/${deleteAlert.user.id}`, { method: 'DELETE' });
              ...
              {initialUsers.length > 0 ? (
                initialUsers.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <Badge variant={user.role === 'admin' ? 'default' : 'secondary'} className="gap-1.5 pl-2">
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <span className="font-semibold">&quot;{deleteAlert.user?.full_name}&quot;</span>{' '}
...
function UserModal({ isOpen, onClose, user, onSuccess, currentUserIsSuperAdmin }: UserModalProps) {
  ...
  const onSubmit = async (formData: UserFormData) => {
    ...
    } else if (!isEditMode && (!payload.password || payload.password.length < 6)) {
      form.setError("password", { message: "Kata sandi minimal 6 karakter." });
      return;
    }

    const toastId = toast.loading(isEditMode ? 'Memperbarui data...' : 'Menambahkan pengguna...');
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Isi detail di bawah ini. Email harus unik dan kata sandi minimal 6 karakter.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
            <FormField control={form.control} name="full_name" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input {...field} placeholder="Nama lengkap pengguna" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="email" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input type="email" {...field} placeholder="email@contoh.com" disabled={isEditMode} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="password" render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>{isEditMode ? 'Kosongkan jika tidak ingin mengubah.' : 'Minimal 6 karakter.'}</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
            {currentUserIsSuperAdmin && (
