...
// --- Utility Functions (Memoized for consistency) ---
...
// --- Data Fetching Components (Optimized) ---

// ✅ Menggunakan React `cache` untuk mendeduplikasi request jika diperlukan di tempat lain.
...
// ✅ Menggunakan satu panggilan RPC, bukan lima panggilan terpisah. Jauh lebih cepat.
...
const RecentActivity = cache(async () => {
  try {
    ...
    const { data: recentEntries, error } = await supabase
      .from('hki')
      .select('id_hki, nama_hki, pemohon(nama_pemohon), status_hki(nama_status)')
...
const StatsCardsSkeleton = () => (
  <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 animate-pulse">
    {[...Array(4)].map((_, i) => <Skeleton key={i} className="h-[125px] w-full rounded-xl" />)}
  </div>
)
