...
export const MasterCrudTable = memo(function MasterCrudTable<
  T extends AnyMasterItem,
>({ dataType, data, config }: MasterCrudTableProps<T>) {
  ...
  const openModal = useCallback(
    (item?: T) => setModalState({ isOpen: true, item }),
    []
  )
  const closeModal = useCallback(
    () => setModalState({ isOpen: false, item: undefined }),
    []
  )
  const openDeleteAlert = useCallback(
    (item: T) => setDeleteAlert({ isOpen: true, item }),
    []
  )
  const closeDeleteAlert = useCallback(
    () => setDeleteAlert({ isOpen: false, item: undefined }),
    []
  )
  ...
  const canAddNew = useMemo(
    () => ['pengusul', 'jenis_hki', 'kelas_hki'].includes(dataType),
    [dataType]
  )
      ...
      <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <CardDescription className="mt-1">
            {config.description} Total {data.length} item.
          </CardDescription>
        </div>
        <TooltipProvider>
          <Tooltip>
...
const FormFields: Record<
  MasterDataType,
  React.FC<{ control: UseFormReturn<any>['control']; isEditMode?: boolean }>
> = {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? 'Edit' : 'Tambah'} {config.title}
          </DialogTitle>
          <DialogDescription>
        ...
        <Form {...form}>
          ...
          >
            {FormComponent && (
              <FormComponent control={form.control} isEditMode={isEditMode} />
            )}
            <DialogFooter>
              <Button
                type="button"
